...
class Viewer(mp.Process):
    def __init__(self, shared_memory: DictProxy[str, Any]) -> None:
        super().__init__()
...
class Robot(mp.Process):
        ...
        if int(self.id) == 0:
            while True:
                ...
                if comando == "quit":
                    break
                di, dj = comando
                self.move_player(di, dj)  # type: ignore
        else:
            while True:
                self.move_npc()

    def move_player(self, di, dj):  # type: ignore
        with self.shared_memory["robots_mutex"]:
            robot = self.shared_memory["robots"][0]
            new_i = robot["i"] + di  # type: ignore
            new_j = robot["j"] + dj  # type: ignore
            if (
                0 <= new_i <= GRID_SIZE[0] - 1
                and 0 <= new_j <= GRID_SIZE[1] - 1
            ):
                with self.shared_memory[
                    "grid_mutex"
                ]:  # Seguro a grid apenas um modifica
                    cell = self.shared_memory["grid"][new_i][new_j]  # type: ignore
                    if cell == "#":
                        print(
                            f"Movimento inválido! O robô {robot['id']} encontrou um obstáculo."
                        )
                        return
                    robot["E"] -= 1  # Consome energia ao mover
                    if robot["E"] <= 0:
                        print(
                            f"Energia esgotada! O robô {robot['id']} não pode se mover."
                        )
                        return
                    robot["i"] = new_i
                    robot["j"] = new_j
                    ...
                    if cell == "*":
                        ...
                        self.esta_proximo()
                    logging.info(
                        f"Robô {robot['id']} se moveu para ({new_i}, {new_j})!"
                    )
    ...
    def move_npc(self):
        with self.shared_memory["robots_mutex"]:
            ...
            direcao = direcoes.pop()
            new_i = robot["i"] + direcao[0]
            new_j = robot["j"] + direcao[1]

            if 0 <= new_i < GRID_SIZE[0] - 1 and 0 <= new_j < GRID_SIZE[1] - 1:
                with self.shared_memory["grid_mutex"]:
                    ...
                    logging.info(
                        f"NPC {robot['id']} se moveu para ({new_i}, {new_j})!"
                    )
